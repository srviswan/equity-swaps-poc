...
def _employee_has_required_skills(emp_row, req_skills):
    ...
    Returns True if employee has at least one required technical skill AND 
    at least one required functional skill (if both are specified).
    If only one type is specified, employee just needs that type.
    """
    ...
    # Check if employee has at least one required technical skill
    has_tech = False
    if req_skills.get('technical'):
        ...
        has_tech = True  # No technical requirement
    
    # Check if employee has at least one required functional skill
    has_func = False
    if req_skills.get('functional'):
        for func in req_skills.get('functional', []):
            if func.lower() in emp_func:
                has_func = True
                break
    else:
        has_func = True  # No functional requirement
    
    return has_tech and has_func
