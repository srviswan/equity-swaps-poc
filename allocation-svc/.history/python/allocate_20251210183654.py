...
def simple_fractional_allocator(employees_df, projects_df, scenario_id, global_start=None, global_end=None):
    ...
    # Ensure projects get staffed if budget allows. Use a small minimum (0.1 FTE) or 
    # calculate based on budget availability
    for pid, month in project_months:
        proj_info = project_data[pid]
        budget = proj_info['per_month_budget']
        
        # Calculate minimum feasible allocation based on cheapest available employee
        min_alloc = 0.0
        if budget > 0:
            # Find cheapest employee who can work on this project
            ...
            # If we have budget, try to allocate at least 0.1 FTE if possible
