...
def fully_optimized_allocator(
    ...
    # Workload leveling variables (month-to-month change)
    leveling_vars = {}
    if weights['leveling_weight'] > 0:
        for eid in employee_ids:
            for month_idx in range(1, len(all_months)):
                ...
                # Calculate total allocation change for this employee
                level_var = solver.NumVar(0.0, 2.0, f'level_e{eid}_m{month}')
                leveling_vars[(eid, month)] = level_var
                
                # Sum of allocations this month
                constraint1 = solver.Constraint(0.0, solver.infinity(), f'level_sum1_e{eid}_m{month}')
                ...
                # Sum of allocations previous month
                constraint2 = solver.Constraint(0.0, solver.infinity(), f'level_sum2_e{eid}_m{month}')
