...
def _employee_has_required_skills(emp_row, req_skills):
    """Check if employee has all required skills for a project"""
    ...
    for tech in req_skills.get('technical', []):
        if tech.lower() not in emp_tech:
            return False
    
    # Check functional skills
    for func in req_skills.get('functional', []):
        if func.lower() not in emp_func:
            return False
