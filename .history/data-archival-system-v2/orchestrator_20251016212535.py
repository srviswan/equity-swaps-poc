...
class ArchivalOrchestrator:
    ...
    def complete_archival_workflow(self, source_db: str, table_name: str) -> bool:
        """Complete archival workflow: mark, move, and cleanup in one step"""
        logger.info(f"Running complete archival workflow for {source_db}.{table_name}")
        
        return self.sql.execute_procedure(
            "control_db",
            "control.sp_Complete_Archival_Workflow",
            {
                'source_database': source_db,
                'table_name': table_name,
                'batch_id': self.batch_id
    ...
    def run_monthly_archival(self):
        ...
        for table_config in tables:
            ...
            # Step 1: Mark records for archival
            if not self.mark_records_for_archival(source_db, table_name):
                logger.error(f"Failed to mark records for {source_db}.{table_name}")
                continue
            
            # Step 2: Move to monthly archival tables
            archival_months = self.get_archival_months(source_db, table_name)
            if not archival_months:
                logger.info(f"No records marked for archival in {source_db}.{table_name}")
                continue
            
            logger.info(f"Found {len(archival_months)} archival months: {', '.join(archival_months)}")
            
            for month in archival_months:
                if not self.move_to_monthly_archival(source_db, table_name, month):
                    logger.error(f"Failed to move {month} data for {source_db}.{table_name}")
                    continue
        
        # Step 3: Move old monthly tables to archive DB
        logger.info("=" * 80)
        logger.info("Moving old monthly archival tables to archive_db")
        
        monthly_tables = self.get_monthly_tables_to_move()
        logger.info(f"Found {len(monthly_tables)} monthly tables ready to move to archive_db")
        
        for monthly_table in monthly_tables:
            if not self.move_to_archive_db(
                monthly_table['source_database'],
                monthly_table['table_name'],
                monthly_table['archival_month']
            ):
                logger.error(f"Failed to move {monthly_table['monthly_table_name']} to archive_db")
