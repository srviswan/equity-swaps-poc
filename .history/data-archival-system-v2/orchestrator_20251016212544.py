...
class ArchivalOrchestrator:
    ...
    def run_monthly_archival(self):
        ...
        # Process each table using comprehensive archival approach
        for table_config in tables:
            ...
            # Use comprehensive archival workflow (mark, move, cleanup in one step)
            if not self.complete_archival_workflow(source_db, table_name):
                logger.error(f"Failed to archive {source_db}.{table_name}")
                continue
            
            logger.info(f"Successfully processed {source_db}.{table_name}")
