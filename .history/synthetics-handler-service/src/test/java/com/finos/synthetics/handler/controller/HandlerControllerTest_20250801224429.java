...
class HandlerControllerTest {
    ...
    void setUp() {
        ...
        response = InstructionResponse.builder()
                .instructionId("test-instruction-id")
                .status(InstructionResponse.Status.SUCCESS)
                .message("Execution processed successfully")
                .result("Execution processed successfully for instruction: test-instruction-id")
    ...
    void shouldProcessInstructionUsingStrategy() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/handler/process")
                ...
                .andExpect(jsonPath("$.message").value("Execution processed successfully"))
    ...
    void shouldHandleProcessingError() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/handler/process")
                ...
                .andExpect(jsonPath("$.message").value("Processing failed: Processing error"));
    ...
    void shouldReturnHealthStatus() throws Exception {
        mockMvc.perform(get("/api/v1/handler/health"))
                .andExpect(status().isOk())
                .andExpect(content().string("Handler Service is running"));
    }
    ...
    void shouldReturnServiceInfo() throws Exception {
        mockMvc.perform(get("/api/v1/handler/info"))
                .andExpect(status().isOk())
                .andExpect(content().string("Handler Service v1.0.0 - CDM Instruction Processing"));
    }
    ...
    void shouldHandleCorsPreflightRequest() throws Exception {
        mockMvc.perform(options("/api/v1/handler/process")
