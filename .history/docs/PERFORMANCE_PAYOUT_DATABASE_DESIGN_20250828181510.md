...
CREATE TABLE cashflows (
    ...
    -- Enhanced status tracking with deferred state
    cashflow_status VARCHAR(50) DEFAULT 'ACCRUED', -- 'ACCRUED', 'REALIZED_DEFERRED', 'REALIZED_UNSETTLED', 'REALIZED_SETTLED'
    realization_date DATE,
    deferral_date DATE, -- When cashflow was deferred
    settlement_date DATE,
    
    -- Deferral details
    deferral_reason VARCHAR(100), -- 'BUSINESS_RULE', 'REGULATORY', 'COUNTERPARTY_REQUEST', 'MARKET_CONDITIONS'
    deferral_period_days INTEGER, -- How long the cashflow is deferred
    expected_realization_date DATE, -- When deferral is expected to end
...
CREATE INDEX idx_cashflows_deferral_date ON cashflows(deferral_date);
CREATE INDEX idx_cashflows_expected_realization_date ON cashflows(expected_realization_date);
```

#### **1.12 Daily Accrual Tracking Table**

```sql
-- Daily granular accrual tracking for time series analysis
CREATE TABLE daily_accruals (
    ...
    business_date DATE NOT NULL, -- Adjusted for business day conventions
    
    -- Accrual amounts
    daily_accrual_amount DECIMAL(19,6) NOT NULL,
    cumulative_accrual_amount DECIMAL(19,6) NOT NULL,
    currency VARCHAR(3) NOT NULL,
    
    -- Accrual calculation details
    accrual_factor DECIMAL(19,6) NOT NULL, -- Day count fraction (e.g., 1/365, 1/252)
    notional_amount DECIMAL(19,6) NOT NULL, -- Notional for accrual calculation
    rate_or_price DECIMAL(19,6), -- Interest rate, price, or other factor
    
    -- Market data snapshot for the date
    market_data_snapshot JSONB, -- Store market data used for daily accrual
    
    -- Business metadata
    is_business_day BOOLEAN NOT NULL,
    business_day_adjustment VARCHAR(50), -- 'FOLLOWING', 'PRECEDING', 'MODIFIED_FOLLOWING'
    ...
    -- Constraints
    CONSTRAINT fk_daily_accruals_cashflow 
        FOREIGN KEY (cashflow_id) REFERENCES cashflows(id),
    CONSTRAINT fk_daily_accruals_contract 
        FOREIGN KEY (contract_id) REFERENCES contracts(id),
    CONSTRAINT fk_daily_accruals_leg 
        FOREIGN KEY (leg_id) REFERENCES contract_legs(id),
    CONSTRAINT uk_daily_accrual_unique UNIQUE (cashflow_id, accrual_date)
);

-- Indexes for performance
CREATE INDEX idx_daily_accruals_cashflow_id ON daily_accruals(cashflow_id);
CREATE INDEX idx_daily_accruals_contract_id ON daily_accruals(contract_id);
CREATE INDEX idx_daily_accruals_leg_id ON daily_accruals(leg_id);
CREATE INDEX idx_daily_accruals_date ON daily_accruals(accrual_date);
CREATE INDEX idx_daily_accruals_business_date ON daily_accruals(business_date);
CREATE INDEX idx_daily_accruals_contract_date ON daily_accruals(contract_id, accrual_date);
```

#### **1.13 Unrealized P&L Time Series Table**

```sql
-- Time series tracking of unrealized P&L for mark-to-market
CREATE TABLE unrealized_pnl_timeseries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    contract_id UUID NOT NULL,
    leg_id UUID NOT NULL,
    ...
    -- Component breakdown
    price_pnl_amount DECIMAL(19,6), -- Price movement P&L
    dividend_pnl_amount DECIMAL(19,6), -- Dividend P&L
    interest_pnl_amount DECIMAL(19,6), -- Interest accrual P&L
    fx_pnl_amount DECIMAL(19,6), -- Foreign exchange P&L
    other_pnl_amount DECIMAL(19,6), -- Other P&L components
    
    -- Market data used for valuation
    market_prices JSONB, -- Underlier prices
    interest_rates JSONB, -- Interest rate data
    fx_rates JSONB, -- Foreign exchange rates
    dividend_data JSONB, -- Dividend information
    
    -- Valuation methodology
    valuation_method VARCHAR(100), -- 'MARK_TO_MARKET', 'MODEL_VALUATION', 'EXTERNAL_PRICE'
    valuation_source VARCHAR(255), -- Source of market data
    ...
    -- Constraints
    CONSTRAINT fk_unrealized_pnl_timeseries_contract 
        FOREIGN KEY (contract_id) REFERENCES contracts(id),
    CONSTRAINT fk_unrealized_pnl_timeseries_leg 
        FOREIGN KEY (leg_id) REFERENCES contract_legs(id),
    CONSTRAINT uk_unrealized_pnl_unique UNIQUE (contract_id, leg_id, valuation_date)
);

-- Indexes for performance
CREATE INDEX idx_unrealized_pnl_timeseries_contract_id ON unrealized_pnl_timeseries(contract_id);
CREATE INDEX idx_unrealized_pnl_timeseries_leg_id ON unrealized_pnl_timeseries(leg_id);
CREATE INDEX idx_unrealized_pnl_timeseries_date ON unrealized_pnl_timeseries(valuation_date);
CREATE INDEX idx_unrealized_pnl_timeseries_business_date ON unrealized_pnl_timeseries(business_date);
CREATE INDEX idx_unrealized_pnl_timeseries_contract_date ON unrealized_pnl_timeseries(contract_id, valuation_date);
CREATE INDEX idx_unrealized_pnl_timeseries_eod ON unrealized_pnl_timeseries(is_end_of_day) WHERE is_end_of_day = TRUE;
```

#### **1.14 Cashflow Status History Table**

```sql
-- Track cashflow status changes including deferral for audit trail
CREATE TABLE cashflow_status_history (
    ...
    -- Constraints
    CONSTRAINT fk_cashflow_status_history_cashflow 
        FOREIGN KEY (cashflow_id) REFERENCES cashflows(id)
);

-- Indexes
CREATE INDEX idx_cashflow_status_history_cashflow_id ON cashflow_status_history(cashflow_id);
CREATE INDEX idx_cashflow_status_history_changed_at ON cashflow_status_history(changed_at);
CREATE INDEX idx_cashflow_status_history_from_status ON cashflow_status_history(from_status);
CREATE INDEX idx_cashflow_status_history_to_status ON cashflow_status_history(to_status);
```
