...
### 1. **Cashflow Status Tracking Queries**

#### **Get All Accrued Cashflows for a Contract**
```sql
SELECT 
...
WHERE c.contract_id = 'contract-uuid'
  AND c.cashflow_status = 'ACCRUED'
ORDER BY c.payment_date;
```

#### **Get Realized Unsettled Cashflows**
```sql
SELECT 
    c.cashflow_reference,
    c.amount,
    ...
    c.realization_date,
    c.payment_date,
    DATEDIFF('day', c.realization_date, c.payment_date) as days_since_realization
FROM cashflows c
WHERE c.cashflow_status = 'REALIZED_UNSETTLED'
  AND c.payment_date >= CURRENT_DATE
ORDER BY c.payment_date;
```

#### **Get Settled Cashflows for a Date Range**
```sql
SELECT 
    c.contract_id,
    c.cashflow_type,
    c.cashflow_subtype,
    SUM(c.amount) as total_settled_amount,
    c.currency,
    COUNT(*) as cashflow_count
FROM cashflows c
WHERE c.cashflow_status = 'REALIZED_SETTLED'
  AND c.settlement_date BETWEEN '2024-01-01' AND '2024-01-31'
GROUP BY c.contract_id, c.cashflow_type, c.cashflow_subtype, c.currency
ORDER BY total_settled_amount DESC;
```

### 2. **Payment Processing Queries**
...
SELECT 
    ...
    p2.party_name as receiver
FROM payments p
JOIN cashflows c ON p.cashflow_id = c.id
JOIN parties p1 ON c.payer_party_id = p1.id
JOIN parties p2 ON c.receiver_party_id = p2.id
WHERE p.payment_status = 'PENDING'
  AND c.payment_date = CURRENT_DATE
ORDER BY p.payment_priority DESC, c.payment_date;
```
...
WHERE p.payment_status = 'FAILED'
  AND psh.to_status = 'FAILED'
  AND psh.changed_at >= CURRENT_DATE - INTERVAL '7 days'
ORDER BY psh.changed_at DESC;
```

#### **Get Payment Instructions for Settlement**
```sql
SELECT 
    pi.instruction_type,
    ...
    pi.payer_bank_name,
    pi.payer_bank_swift,
    pi.receiver_bank_name,
    pi.receiver_bank_swift,
    pi.priority,
    pi.urgency
FROM payment_instructions pi
JOIN payments p ON pi.payment_id = p.id
WHERE pi.instruction_status = 'PENDING'
  AND p.payment_status = 'PENDING'
ORDER BY pi.priority DESC, pi.urgency DESC;
```

### 3. **Netting and Settlement Queries**

#### **Get Netting Instructions for a Party**
```sql
SELECT 
    ni.netting_reference,
    ni.netting_type,
    ni.netting_status,
    ni.netting_date,
    ni.netting_currency,
    ...
    COUNT(ncm.cashflow_id) as cashflow_count
FROM netting_instructions ni
JOIN parties p1 ON ni.party1_id = p1.id
JOIN parties p2 ON ni.party2_id = p2.id
LEFT JOIN netting_cashflow_mapping ncm ON ni.id = ncm.netting_instruction_id
WHERE ni.party1_id = 'party-uuid' OR ni.party2_id = 'party-uuid'
GROUP BY ni.id, ni.netting_reference, ni.netting_type, ni.netting_status, 
         ni.netting_date, ni.netting_currency, ni.net_amount, p1.party_name, p2.party_name
ORDER BY ni.netting_date DESC;
```

#### **Calculate Net Position for Two Parties**
```sql
WITH party_cashflows AS (
    SELECT 
        c.payer_party_id as party_id,
        SUM(c.amount) as total_payable
    FROM cashflows c
    WHERE c.cashflow_status IN ('ACCRUED', 'REALIZED_UNSETTLED')
      AND c.payer_party_id IN ('party1-uuid', 'party2-uuid')
    GROUP BY c.payer_party_id
    
    UNION ALL
    
    SELECT 
        c.receiver_party_id as party_id,
        -SUM(c.amount) as total_payable
    FROM cashflows c
    WHERE c.cashflow_status IN ('ACCRUED', 'REALIZED_UNSETTLED')
      AND c.receiver_party_id IN ('party1-uuid', 'party2-uuid')
    GROUP BY c.receiver_party_id
)
SELECT 
    p.party_name,
    SUM(pc.total_payable) as net_position
FROM party_cashflows pc
...
#### **Daily Cashflow Summary Dashboard**
```sql
SELECT 
...
GROUP BY DATE(c.calculation_date), c.cashflow_type, c.cashflow_subtype, 
         c.cashflow_status, c.currency
...
SELECT 
    ...
    AVG(DATEDIFF('day', p.payment_instruction_date, p.payment_settlement_date)) as avg_settlement_days,
    ...
    (SUM(CASE WHEN p.payment_status = 'COMPLETED' THEN 1 ELSE 0 END) * 100.0 / COUNT(*)) as success_rate
FROM payments p
WHERE p.payment_execution_date >= CURRENT_DATE - INTERVAL '90 days'
GROUP BY p.payment_method, p.payment_currency
ORDER BY total_volume DESC;
```

#### **Contract Cashflow Aging Analysis**
```sql
SELECT 
    c.contract_id,
    c.cashflow_type,
    ...
    c.currency,
    CASE 
        WHEN c.cashflow_status = 'ACCRUED' THEN 
            DATEDIFF('day', c.accrual_end_date, CURRENT_DATE)
        WHEN c.cashflow_status = 'REALIZED_UNSETTLED' THEN 
            DATEDIFF('day', c.realization_date, CURRENT_DATE)
        ELSE 0
    END as days_outstanding
FROM cashflows c
WHERE c.cashflow_status IN ('ACCRUED', 'REALIZED_UNSETTLED')
GROUP BY c.contract_id, c.cashflow_type, c.cashflow_subtype, c.cashflow_status, c.currency
HAVING days_outstanding > 0
ORDER BY days_outstanding DESC, total_amount DESC;
```

## **Business Use Cases**

### 1. **Daily Cashflow Generation**
- Generate accrual cashflows for all active contracts
- Calculate realized amounts based on market data
- Update cashflow statuses based on business rules

### 2. **Payment Processing**
- Process scheduled payments for due cashflows
- Handle payment instructions and bank routing
- Track payment status and settlement confirmations

### 3. **Netting and Settlement**
- Calculate net positions between counterparties
- Generate netting instructions for eligible cashflows
- Process settlement through clearing systems

### 4. **Risk and Compliance**
- Monitor outstanding cashflows and aging
- Track payment failures and investigate issues
- Generate regulatory reports on payment activities

### 5. **Reporting and Analytics**
- Daily cashflow position reports
