...
def format_excel_file(writer, df_transactions, df_summary, df_fund_performance, df_monthly):
    ...
    fund_sheet.set_column('A:A', 10)  # Portfolio (if present)
    fund_sheet.set_column('B:B', 35)  # Fund Name
    fund_sheet.set_column('C:C', 15)  # ISIN
    fund_sheet.set_column('D:D', 15, money_format)  # Total Invested
    fund_sheet.set_column('E:E', 15, money_format)  # Total Received
    fund_sheet.set_column('F:F', 12, money_format)  # Net P&L
    fund_sheet.set_column('G:G', 10, percent_format)  # Return %
    fund_sheet.set_column('H:I', 12)  # Shares
    fund_sheet.set_column('J:J', 15)  # Remaining Position
    fund_sheet.set_column('K:L', 12, money_format)  # Avg Prices
    ...
    monthly_sheet.set_column('A:A', 10)  # Portfolio (if present)
    ...
    monthly_sheet.set_column('E:E', 12)  # Total Shares
    monthly_sheet.set_column('F:F', 12)  # Funds Traded
...
def main():
    print("🔍 Processing both GIA and ISA portfolios with reference numbers...")
    print("="*60)
    
    # Portfolio reference numbers extracted from PDFs
    portfolio_refs = {
        "gia_transactions_*.csv": "IP00839191",  # GIA Account Reference
        "isa_transactions_*.csv": "IP00465009"   # ISA Account Reference  
    }
    
    # Process GIA portfolio
    print("\n📊 PROCESSING GIA PORTFOLIO (IP00839191)")
    print("-" * 40)
    gia_file = process_portfolio("gia_transactions_*.csv", "IP00839191")
    
    # Process ISA portfolio  
    print("\n📊 PROCESSING ISA PORTFOLIO (IP00465009)")
    print("-" * 40)
    isa_file = process_portfolio("isa_transactions_*.csv", "IP00465009")
    ...
    if gia_file:
        print(f"✅ GIA Analysis (IP00839191): {gia_file}")
    if isa_file:
        print(f"✅ ISA Analysis (IP00465009): {isa_file}")
        
    if not gia_file and not isa_file:
        print("❌ No portfolio files were created successfully!")
    
    print("\n📁 Both portfolios now have comprehensive Excel analysis with reference numbers!")
    print("💼 Portfolio References:")
    print("   📊 GIA Account: IP00839191")
    print("   📊 ISA Account: IP00465009")
