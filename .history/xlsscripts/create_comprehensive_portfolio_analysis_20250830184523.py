...
def create_monthly_summary_by_portfolio(df):
    ...
    # Create year-month string directly
    df_copy = df.copy()
    df_copy['year_month'] = df_copy['date'].dt.strftime('%Y-%m')
    
    monthly_summary = df_copy.groupby(['portfolio_reference', 'year_month', 'action']).agg({
        'amount': 'sum',
        'quantity': 'sum',
        'isin': 'nunique'
    }).reset_index()
...
def create_comprehensive_analysis():
    ...
    # Clean data for Excel (handle NaN/INF values and Period objects)
    print("Cleaning data for Excel export...")
    
    # Convert any Period objects to strings
    for col in df_transactions.columns:
        if df_transactions[col].dtype == 'object':
            df_transactions[col] = df_transactions[col].astype(str)
    
    for col in df_monthly.columns:
        if df_monthly[col].dtype == 'object':
            df_monthly[col] = df_monthly[col].astype(str)
    
    df_transactions = df_transactions.fillna('')
    df_portfolio_summary = df_portfolio_summary.fillna('')
    df_fund_performance = df_fund_performance.fillna('').replace([np.inf, -np.inf], '')
