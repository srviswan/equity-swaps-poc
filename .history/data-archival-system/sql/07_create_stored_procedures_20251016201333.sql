...
-- Generic bulk copy archival procedure with filegroup support
...
BEGIN
    ...
    SET @dst = @archive_db_ref + N'.' + QUOTENAME(@source_db) + N'.' + QUOTENAME(@table_name);
    ...
    DECLARE @cols NVARCHAR(MAX) = (
        ...
        WHERE s.name=@table_schema AND t.name=@table_name
          AND name NOT IN ('archived_date','archive_batch_id','retention_expiry_date','archival_flag')
    );
    
    DECLARE @pk_join NVARCHAR(MAX) = control.fn_Build_PK_Join(@pk_csv, 's', 'd');
    
    -- Insert with idempotent check using archival_flag
    DECLARE @insert NVARCHAR(MAX) = N'INSERT INTO ' + @dst +
        N' (' + @cols + N', archived_date, archival_batch_id, retention_expiry_date) ' +
        N' SELECT ' + @cols + N', GETDATE(), @batch_id, DATEADD(YEAR, @years_retention, GETDATE())' +
        N' FROM ' + @src + N' s ' +
        N' WHERE s.archival_flag = 1 ' +
        N' AND NOT EXISTS (SELECT 1 FROM ' + @dst + N' d WHERE ' + @pk_join + N');';
    
    EXEC sp_executesql @insert,
        N'@batch_id UNIQUEIDENTIFIER, @years_retention INT',
        @batch_id=@batch_id, @years_retention=@years_retention;
    
    DECLARE @rows_archived INT = @@ROWCOUNT;
    
    -- Update archival_marker with archived status
    UPDATE control.archival_marker
    SET archived_date = GETDATE(), batch_id = @batch_id
    WHERE source_database = @source_db AND table_schema = @table_schema AND table_name = @table_name
      AND archival_eligible = 1 AND archived_date IS NULL;
    
    -- Log maintenance activity
    EXEC control.sp_Log_Maintenance_Activity
        @maintenance_type = 'ARCHIVAL',
        @filegroup_name = 'ARCHIVE_FG',
        @start_time = @start_time,
        @end_time = GETDATE(),
        @status = 'SUCCESS',
        @records_processed = @rows_archived;
    
    PRINT 'Bulk copy archival completed for ' + @source_db + '.' + @table_schema + '.' + @table_name + 
          ' (' + CAST(@rows_archived AS VARCHAR) + ' rows)';
END;
GO
