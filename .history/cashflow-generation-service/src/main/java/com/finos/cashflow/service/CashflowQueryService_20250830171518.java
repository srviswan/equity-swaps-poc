...
import com.finos.cashflow.model.CashflowPageResponse;
import com.finos.cashflow.model.CashflowStatus;
import com.finos.cashflow.model.CashflowType;
import com.finos.cashflow.model.DailyAccrual;
import com.finos.cashflow.model.DailyAccrualPageResponse;
import com.finos.cashflow.model.UnrealizedPnL;
import com.finos.cashflow.repository.CashflowRepository;
import com.finos.cashflow.repository.DailyAccrualRepository;
import com.finos.cashflow.repository.UnrealizedPnLRepository;
...
public class CashflowQueryService {
     ...
     */
    public Flux<com.finos.cashflow.model.DailyAccrual> getInterestAccruals(UUID contractId, LocalDate startDate, LocalDate endDate) {
     ...
     */
    public Flux<com.finos.cashflow.model.DailyAccrual> getDividendAccruals(UUID contractId, LocalDate startDate, LocalDate endDate) {
     ...
     */
    public Flux<com.finos.cashflow.model.DailyAccrual> getPerformanceAccruals(UUID contractId, LocalDate startDate, LocalDate endDate) {
     ...
     */
    public Flux<com.finos.cashflow.model.UnrealizedPnL> getUnrealizedPnLForPerformance(UUID contractId, String securityId, LocalDate startDate, LocalDate endDate) {
    ...
    // ===== New API Query Methods =====
    
    /**
     * Search cashflows with filters and pagination support.
     */
    public Mono<CashflowPageResponse> searchCashflows(UUID contractId, UUID legId, CashflowType cashflowType, 
                                                     CashflowStatus status, String currency, LocalDate startDate, 
                                                     LocalDate endDate, int page, int size) {
        int offset = page * size;
        
        String cashflowTypeStr = cashflowType != null ? cashflowType.name() : null;
        String statusStr = status != null ? status.name() : null;
        
        Mono<List<Cashflow>> contentMono = cashflowRepository
                .findCashflowsWithFilters(contractId, legId, cashflowTypeStr, statusStr, currency, 
                                         startDate, endDate, size, offset)
                .collectList();
        
        Mono<Long> totalMono = cashflowRepository
                .countCashflowsWithFilters(contractId, legId, cashflowTypeStr, statusStr, currency, 
                                          startDate, endDate);
        
        return Mono.zip(contentMono, totalMono)
                .map(tuple -> {
                    List<Cashflow> content = tuple.getT1();
                    Long total = tuple.getT2();
                    
                    // Create a Page-like object for CashflowPageResponse
                    Page<Cashflow> page1 = new PageImpl<>(content, PageRequest.of(page, size), total);
                    return new CashflowPageResponse(page1);
                });
    }
    ...
    /**
     * Get daily accruals for a specific contract with pagination.
     */
    public Mono<DailyAccrualPageResponse> getDailyAccruals(UUID contractId, LocalDate startDate, 
                                                          LocalDate endDate, int page, int size) {
        int offset = page * size;
        
        Mono<List<DailyAccrual>> contentMono = dailyAccrualRepository
                .findByContractIdAndAccrualDateBetween(contractId, startDate, endDate)
                .skip(offset)
                .take(size)
                .collectList();
        
        Mono<Long> totalMono = dailyAccrualRepository
                .countByContractIdAndAccrualDateBetween(contractId, startDate, endDate);
        
        return Mono.zip(contentMono, totalMono)
                .map(tuple -> {
                    List<DailyAccrual> content = tuple.getT1();
                    Long total = tuple.getT2();
                    
                    Page<DailyAccrual> page1 = new PageImpl<>(content, PageRequest.of(page, size), total);
                    return new DailyAccrualPageResponse(page1);
                });
    }
    
    /**
     * Get deferred cashflows for aging analysis.
     */
    public Flux<Cashflow> getDeferredCashflows(LocalDate startDate, LocalDate endDate) {
        return cashflowRepository.findDeferredCashflows(startDate, endDate);
    }
    
    /**
     * Get overdue deferred cashflows.
     */
    public Flux<Cashflow> getOverdueDeferredCashflows() {
        return cashflowRepository.findOverdueDeferredCashflows(LocalDate.now());
    }
    
    /**
     * Get cashflows by currency.
     */
    public Flux<Cashflow> getCashflowsByCurrency(String currency) {
        return cashflowRepository.findByCurrency(currency);
    }
    
    /**
     * Get cashflows by leg ID.
     */
    public Flux<Cashflow> getCashflowsByLegId(UUID legId) {
        return cashflowRepository.findByLegId(legId);
    }
}
