...
public class CashflowGenerationService {
    ...
    public Mono<CashflowGenerationResponse> generateCashflows(CashflowGenerationRequest request) {
        ...
        List<CompletableFuture<Cashflow>> futures = request.getContractIds().stream()
            .map(contractId -> threadPartitioningService.executeInPartition(
                createPartitionKey(contractId, request.getPrimaryCalculationType()),
    ...
    /**
     * Create a partition key for thread partitioning.
     * 
     * @param contractId Contract ID
     * @param calculationType Calculation type
     * @return Partition key
     */
    private ThreadPartitionKey createPartitionKey(UUID contractId, CalculationType calculationType) {
        return new ThreadPartitionKey(contractId, "DEFAULT", calculationType);
    }
    ...
    private Cashflow generateCashflowForContract(UUID contractId, CashflowGenerationRequest request) {
        ...
        return new Cashflow(
            ...
            request.getPrimaryCalculationType().name(),
