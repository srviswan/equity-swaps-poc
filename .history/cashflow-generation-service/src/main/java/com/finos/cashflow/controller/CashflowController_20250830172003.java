...
public class CashflowController {
    ...
    public Mono<ResponseEntity<Cashflow>> updateCashflowStatus(
        ...
        logger.info("Updating cashflow {} status to {}", cashflowId, request.getNewStatus());
        
        return cashflowQueryService.getCashflowById(cashflowId)
                .flatMap(cashflow -> {
                    // Use the state management service to perform transition
                    return cashflowStateManagementService.transitionStatus(cashflow, request.getNewStatus(), 
                                                                          request.getReason(), "API_USER");
                })
                .cast(Cashflow.class)
                .map(ResponseEntity::ok)
    ...
    public Mono<ResponseEntity<Cashflow>> settleCashflow(
        ...
        logger.info("Settling cashflow {} on {}", cashflowId, request.getSettlementDate());
    ...
    /**
     * Batch generate cashflows.
     */
    @PostMapping("/cashflows/generate/batch")
    public Mono<ResponseEntity<BatchCashflowGenerationResponse>> batchGenerateCashflows(
            @RequestBody BatchCashflowGenerationRequest request) {
        
        logger.info("Received batch cashflow generation request with {} requests", request.getRequests().size());
        
        UUID jobId = UUID.randomUUID();
        int totalRequests = request.getRequests().size();
        
        // Process each request in the batch
        Flux<CashflowGenerationResponse> responses = Flux.fromIterable(request.getRequests())
                .flatMap(cashflowGenerationService::generateCashflows)
                .onErrorResume(error -> {
                    logger.error("Error in batch processing", error);
                    return Mono.just(new CashflowGenerationResponse(
                        UUID.randomUUID(), 0, "Error: " + error.getMessage()
                    ));
                });
        
        return responses.collectList()
                .map(responseList -> {
                    int successful = (int) responseList.stream()
                            .filter(r -> !r.getMessage().startsWith("Error:"))
                            .count();
                    int failed = totalRequests - successful;
                    
                    return new BatchCashflowGenerationResponse(jobId, totalRequests, successful, failed);
                })
                .map(response -> ResponseEntity.accepted().body(response));
    }
    
    /**
     * Batch generate interest cashflows.
     */
    @PostMapping("/cashflows/generate/interest/batch")
    public Mono<ResponseEntity<BatchCashflowGenerationResponse>> batchGenerateInterestCashflows(
            @RequestBody InterestCashflowGenerationRequest request) {
        
        logger.info("Received batch interest cashflow generation for {} contracts", 
                   request.getContractIds().size());
        
        UUID jobId = UUID.randomUUID();
        
        // Convert to individual requests for each contract
        Flux<CashflowGenerationResponse> responses = Flux.fromIterable(request.getContractIds())
                .flatMap(contractId -> {
                    CashflowGenerationRequest individualRequest = new CashflowGenerationRequest();
                    individualRequest.setContractIds(List.of(contractId));
                    individualRequest.setCalculationDate(request.getCalculationDate());
                    individualRequest.setCashflowTypes(List.of(CashflowType.INTEREST));
                    
                    return cashflowGenerationService.generateInterestCashflows(individualRequest);
                })
                .onErrorResume(error -> {
                    logger.error("Error in batch interest processing", error);
                    return Mono.just(new CashflowGenerationResponse(
                        UUID.randomUUID(), 0, "Error: " + error.getMessage()
                    ));
                });
        ...
        return responses.collectList()
                .map(responseList -> {
                    int totalRequests = request.getContractIds().size();
                    int successful = (int) responseList.stream()
                            .filter(r -> !r.getMessage().startsWith("Error:"))
                            .count();
                    int failed = totalRequests - successful;
    ...
    /**
     * Batch generate equity cashflows.
     */
    @PostMapping("/cashflows/generate/equity/batch")
    public Mono<ResponseEntity<BatchCashflowGenerationResponse>> batchGenerateEquityCashflows(
            @RequestBody PerformanceCashflowGenerationRequest request) {
        
        logger.info("Received batch equity cashflow generation for {} contracts", 
                   request.getContractIds().size());
        ...
        Flux<CashflowGenerationResponse> responses = Flux.fromIterable(request.getContractIds())
                .flatMap(contractId -> {
                    CashflowGenerationRequest individualRequest = new CashflowGenerationRequest();
                    individualRequest.setContractIds(List.of(contractId));
                    individualRequest.setCalculationDate(request.getValuationDate());
                    individualRequest.setCashflowTypes(List.of(CashflowType.PERFORMANCE, CashflowType.DIVIDEND));
                    
                    return cashflowGenerationService.generatePerformanceCashflows(individualRequest);
                })
                .onErrorResume(error -> {
                    logger.error("Error in batch equity processing", error);
                    return Mono.just(new CashflowGenerationResponse(
                        UUID.randomUUID(), 0, "Error: " + error.getMessage()
                    ));
                });
    ...
    /**
     * Batch status update for multiple cashflows.
     */
    @PostMapping("/cashflows/batch/status-update")
    public Mono<ResponseEntity<BatchStatusUpdateResponse>> batchUpdateStatus(
            @RequestBody BatchStatusUpdateRequest request) {
        
        logger.info("Received batch status update request for {} cashflows", request.getUpdates().size());
        
        UUID jobId = UUID.randomUUID();
        int totalUpdates = request.getUpdates().size();
        
        Flux<Cashflow> updatedCashflows = Flux.fromIterable(request.getUpdates())
                .flatMap(update -> {
                    return cashflowQueryService.getCashflowById(update.getCashflowId())
                            .flatMap(cashflow -> {
                                return cashflowStateManagementService.transitionStatus(
                                    cashflow, update.getNewStatus(), update.getReason(), "BATCH_API_USER"
                                );
                            })
                            .onErrorResume(error -> {
                                logger.error("Error updating cashflow {}: {}", 
                                           update.getCashflowId(), error.getMessage());
                                return Mono.empty(); // Skip failed updates
                            });
                });
        
        return updatedCashflows.collectList()
                .map(results -> {
                    int successful = results.size();
                    int failed = totalUpdates - successful;
                    
                    return new BatchStatusUpdateResponse(jobId, totalUpdates, successful, failed);
                })
                .map(response -> ResponseEntity.accepted().body(response));
    }
