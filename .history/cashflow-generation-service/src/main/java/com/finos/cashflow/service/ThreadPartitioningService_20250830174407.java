...
import com.finos.cashflow.model.ThreadPartitionStatus;
...
public class ThreadPartitioningService {
     ...
     * 
     * @return ThreadPartitionStatus with current partition information
     */
    public Mono<ThreadPartitionStatus> getPartitionStatus() {
        return Mono.fromCallable(() -> {
            int totalPartitions = partitionExecutors.size();
            ...
            for (ExecutorService executor : partitionExecutors.values()) {
                if (executor != null && !executor.isShutdown()) {
                    if (executor instanceof java.util.concurrent.ThreadPoolExecutor) {
                        ...
                        // For virtual thread executors, assume active if not shutdown
                        activePartitions++;
                    }
                }
            }
            ...
            return new ThreadPartitionStatus(totalPartitions, activePartitions, availablePartitions, queueSize);
        });
    }
}
