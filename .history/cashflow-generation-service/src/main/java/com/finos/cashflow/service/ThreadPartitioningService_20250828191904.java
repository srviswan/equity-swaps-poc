        ...
        return partitionExecutors.computeIfAbsent(partitionString, key -> {
            ...
            if (partitionKey.isInterestPartition()) {
                executor = Executors.newVirtualThreadPerTaskExecutor(
                    r -> {
                        Thread t = Thread.ofVirtual().name("interest-" + partitionString).build();
                        t.setDaemon(true);
                        return t;
                    }
                );
                ...
                executor = Executors.newVirtualThreadPerTaskExecutor(
                    r -> {
                        Thread t = Thread.ofVirtual().name("equity-" + partitionString).build();
                        t.setDaemon(true);
                        return t;
                    }
                );
    ...
    public ConcurrentMap<String, Object> getPartitionStatistics() {
        ...
        partitionExecutors.forEach((partition, executor) -> {
            if (executor instanceof java.util.concurrent.ThreadPoolExecutor tpe) {
                stats.put(partition + ".activeThreads", tpe.getActiveCount());
                stats.put(partition + ".poolSize", tpe.getPoolSize());
                stats.put(partition + ".queueSize", tpe.getQueue().size());
            }
        });
