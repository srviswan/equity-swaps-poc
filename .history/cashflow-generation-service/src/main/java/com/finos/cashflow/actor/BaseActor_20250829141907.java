...
public abstract class BaseActor<T, S> implements Actor<T, S> {
    ...
    public <R> Mono<R> ask(T message, Class<R> responseType) {
        ...
        // Store the response sink temporarily and process the message
        // The message should be processed and a response sent via sendResponse
        messageSink.tryEmitNext(message);
        
        // Return the response with timeout
