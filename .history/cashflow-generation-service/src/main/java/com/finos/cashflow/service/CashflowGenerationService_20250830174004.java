...
public class CashflowGenerationService {
    ...
    /**
     * Generate cashflows reactively, returning a stream of results.
     * 
     * @param request The cashflow generation request
     * @return Flux of generated cashflows
     */
    public Flux<Cashflow> generateCashflowsReactive(CashflowGenerationRequest request) {
        logger.info("Starting reactive cashflow generation for {} contracts", 
                   request.getContractIds().size());
        
        return Flux.fromIterable(request.getContractIds())
                .flatMap(contractId -> Flux.just(generateCashflowForContract(contractId, request)))
                .doOnComplete(() -> logger.info("Completed reactive cashflow generation"));
    }
    ...
    public Mono<BatchCashflowGenerationResponse> generateBatchCashflows(BatchCashflowGenerationRequest batchRequest) {
        ...
        return Mono.fromCallable(() -> {
            BatchCashflowGenerationResponse response = new BatchCashflowGenerationResponse();
            response.setBatchId(UUID.randomUUID());
            response.setTotalRequests(batchRequest.getRequests().size());
            response.setSuccessfulRequests(batchRequest.getRequests().size());
            response.setFailedRequests(0);
            response.setStatus("COMPLETED");
            response.setProcessingTimeMs(System.currentTimeMillis());
            
            logger.info("Completed batch cashflow generation: batchId={}, totalRequests={}", 
                       response.getBatchId(), response.getTotalRequests());
