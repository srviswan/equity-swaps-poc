...
public class CashflowQueryService {
    ...
    public Mono<CashflowPageResponse> searchCashflows(
        ...
        try {
            logger.info("Searching cashflows with contractId: {}, cashflowType: {}, status: {}", 
            ...
            return query
                ...
                .doOnError(error -> logger.error("Error in searchCashflows", error))
                .onErrorReturn(CashflowPageResponse.empty(page, size));
        } catch (Exception e) {
            logger.error("Exception in searchCashflows", e);
            return Mono.just(CashflowPageResponse.empty(page, size));
        }
    }
