...
public class CashflowGenerationService {
    ...
    // Temporarily removed dependencies for testing
    ...
    public CashflowGenerationService() {
        logger.info("MOCK: CashflowGenerationService initialized with mock implementations for testing");
    }
    ...
    public Flux<Cashflow> generateCashflowsReactive(CashflowGenerationRequest request) {
        ...
        // Simple mock implementation that generates sample cashflows
    ...
    public Mono<CashflowGenerationResponse> generateInterestCashflows(CashflowGenerationRequest request) {
        logger.info("MOCK: Generating interest cashflows for {} contracts using thread partitioning", 
                   request.getContractIds().size());
        
        // Simple mock implementation that always works - FORCED REFRESH
        return Mono.just(new CashflowGenerationResponse(
            UUID.randomUUID(),
            request.getContractIds().size(),
            "MOCK: Interest cashflow generation completed using thread partitioning"
        ));
    }
