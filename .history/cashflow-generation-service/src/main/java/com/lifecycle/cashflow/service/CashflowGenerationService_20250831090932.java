...
public class CashflowGenerationService {
    ...
    public Mono<CashflowGenerationResponse> generateCashflows(CashflowGenerationRequest request) {
        ...
        try {
            // Validate request - ensure it has cashflow types for calculation type determination
            final CashflowGenerationRequest finalRequest;
            if (request.getCashflowTypes() == null || request.getCashflowTypes().isEmpty()) {
                logger.warn("Request has null or empty cashflowTypes, defaulting to INTEREST");
                CashflowGenerationRequest newRequest = new CashflowGenerationRequest(
                    request.getContractIds(),
                    request.getCalculationDate(),
                    List.of(CashflowType.INTEREST) // Default to INTEREST type
                );
                newRequest.setBusinessRules(request.getBusinessRules());
            ...
            List<CompletableFuture<Cashflow>> futures = finalRequest.getContractIds().stream()
                .map(contractId -> {
                    ThreadPartitionKey partitionKey = threadPartitioningService.createPartitionKey(
                        contractId, "DEFAULT", finalRequest.getPrimaryCalculationType());
                    return threadPartitioningService.executeInPartition(
                        partitionKey,
                        () -> generateCashflowForContract(contractId, finalRequest)
                    );
                })
                .toList();
            ...
            return Mono.fromFuture(allFutures)
                .then(Mono.just(new CashflowGenerationResponse(
                    UUID.randomUUID(),
                    finalRequest.getContractIds().size(),
                    "Cashflow generation completed using thread partitioning"
                )))
                .doOnError(error -> logger.error("Error in generateCashflows", error))
                .onErrorReturn(new CashflowGenerationResponse(
                    UUID.randomUUID(),
                    finalRequest.getContractIds().size(),
                    "Cashflow generation failed due to error"
                ));
        } catch (Exception e) {
            logger.error("Exception in generateCashflows", e);
            return Mono.just(new CashflowGenerationResponse(
                UUID.randomUUID(),
                request.getContractIds().size(),
                "Cashflow generation failed - " + e.getMessage()
            ));
        }
    }
    ...
    public Mono<CashflowGenerationResponse> generateInterestCashflows(CashflowGenerationRequest request) {
        ...
        try {
            ...
            Mono<CashflowGenerationResponse> result = generateCashflows(interestRequest);
            if (result == null) {
                logger.error("generateCashflows returned null for interest request");
                return Mono.just(new CashflowGenerationResponse(
                    UUID.randomUUID(),
                    request.getContractIds().size(),
                    "Interest cashflow generation failed - service error"
                ));
            }
            return result;
        } catch (Exception e) {
            logger.error("Error in generateInterestCashflows", e);
            return Mono.just(new CashflowGenerationResponse(
                UUID.randomUUID(),
                request.getContractIds().size(),
                "Interest cashflow generation failed - " + e.getMessage()
            ));
        }
    }
