...
public class CashflowServiceIntegrationTest {
    ...
    void testInterestCashflowGeneration() {
        ...
        request.setDayCountConvention(InterestCashflowGenerationRequest.DayCountConvention.ACT_365);
        request.setBusinessDayAdjustment(InterestCashflowGenerationRequest.BusinessDayAdjustment.FOLLOWING);
    ...
    void testActorBasedCashflowGeneration() {
        ...
        // When & Then
        StepVerifier.create(actorBasedCashflowService.generateCashflows(request))
                .assertNext(response -> {
                    assertNotNull(response);
                    assertNotNull(response.getJobId());
                })
    ...
    void testDailyAccrualGeneration() {
        ...
        request.setAccrualTypes(Arrays.asList(DailyAccrualGenerationRequest.AccrualType.INTEREST));
        request.setBusinessDayAdjustment(DailyAccrualGenerationRequest.BusinessDayAdjustment.FOLLOWING);

        // When & Then
        StepVerifier.create(actorBasedCashflowService.generateDailyAccruals(request))
                .assertNext(response -> {
                    assertNotNull(response);
                    assertNotNull(response.getJobId());
                })
    ...
    void testCashflowQueryService() {
        ...
        // When & Then - Test query by cashflow type
    ...
    void testCashflowStateTransitions() {
        ...
        // When & Then - Test status transition
        ...
        statusRequest.setReason("Testing status transition");

        StepVerifier.create(stateManagementService.transitionStatus(testCashflowId, statusRequest))
                .assertNext(updated -> {
                    assertEquals(CashflowStatus.REALIZED_DEFERRED, updated.getStatus());
                })
                .verifyComplete();
    }
    ...
    void testDatabaseConstraintsAndValidation() {
        // Given - Cashflow with minimal valid data
        Cashflow cashflow = new Cashflow();
        cashflow.setId(UUID.randomUUID());
        cashflow.setContractId(UUID.randomUUID());
        ...
        cashflow.setAmount(BigDecimal.valueOf(100.00));
        ...
        StepVerifier.create(cashflowRepository.save(cashflow))
                .assertNext(saved -> {
                    assertNotNull(saved.getId());
                    assertEquals("TEST-SEC-001", saved.getSecurityId());
                })
                .verifyComplete();
    }
