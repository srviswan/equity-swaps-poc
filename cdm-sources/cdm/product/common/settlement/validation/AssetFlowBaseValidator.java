...
public class AssetFlowBaseValidator implements Validator<AssetFlowBase> {
	...
	public ValidationResult<AssetFlowBase> validate(RosettaPath path, AssetFlowBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AssetFlowBase", ValidationType.CARDINALITY, "AssetFlowBase", path, "", error);
		}
		return success("AssetFlowBase", ValidationType.CARDINALITY, "AssetFlowBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetFlowBase", ValidationType.CARDINALITY, "AssetFlowBase", path, "", res.getError());
				}
				return success("AssetFlowBase", ValidationType.CARDINALITY, "AssetFlowBase", path, "");
			})
			.collect(toList());
