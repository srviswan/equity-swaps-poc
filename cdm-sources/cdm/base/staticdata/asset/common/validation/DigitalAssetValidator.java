...
public class DigitalAssetValidator implements Validator<DigitalAsset> {
	...
	public ValidationResult<DigitalAsset> validate(RosettaPath path, DigitalAsset o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DigitalAsset", ValidationType.CARDINALITY, "DigitalAsset", path, "", error);
		}
		return success("DigitalAsset", ValidationType.CARDINALITY, "DigitalAsset", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DigitalAsset", ValidationType.CARDINALITY, "DigitalAsset", path, "", res.getError());
				}
				return success("DigitalAsset", ValidationType.CARDINALITY, "DigitalAsset", path, "");
			})
			.collect(toList());
