...
public interface AssetChoice extends Validator<Asset> {
	...
	ValidationResult<Asset> validate(RosettaPath path, Asset asset);
	
	class Default implements AssetChoice {
	
		@Override
		public ValidationResult<Asset> validate(RosettaPath path, Asset asset) {
			ComparisonResult result = executeDataRule(asset);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Asset", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "Asset", path, DEFINITION, failureMessage);
		}
