...
public class AssetBaseValidator implements Validator<AssetBase> {
	...
	public ValidationResult<AssetBase> validate(RosettaPath path, AssetBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AssetBase", ValidationType.CARDINALITY, "AssetBase", path, "", error);
		}
		return success("AssetBase", ValidationType.CARDINALITY, "AssetBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetBase", ValidationType.CARDINALITY, "AssetBase", path, "", res.getError());
				}
				return success("AssetBase", ValidationType.CARDINALITY, "AssetBase", path, "");
			})
			.collect(toList());
