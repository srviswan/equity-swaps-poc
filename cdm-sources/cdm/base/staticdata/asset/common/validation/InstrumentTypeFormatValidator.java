...
public class InstrumentTypeFormatValidator implements Validator<Instrument> {
	...
	public ValidationResult<Instrument> validate(RosettaPath path, Instrument o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Instrument", ValidationType.TYPE_FORMAT, "Instrument", path, "", error);
		}
		return success("Instrument", ValidationType.TYPE_FORMAT, "Instrument", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Instrument", ValidationType.TYPE_FORMAT, "Instrument", path, "", res.getError());
				}
				return success("Instrument", ValidationType.TYPE_FORMAT, "Instrument", path, "");
			})
			.collect(toList());
