...
public class InstrumentBaseValidator implements Validator<InstrumentBase> {
	...
	public ValidationResult<InstrumentBase> validate(RosettaPath path, InstrumentBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InstrumentBase", ValidationType.CARDINALITY, "InstrumentBase", path, "", error);
		}
		return success("InstrumentBase", ValidationType.CARDINALITY, "InstrumentBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentBase", ValidationType.CARDINALITY, "InstrumentBase", path, "", res.getError());
				}
				return success("InstrumentBase", ValidationType.CARDINALITY, "InstrumentBase", path, "");
			})
			.collect(toList());
