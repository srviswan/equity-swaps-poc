...
public interface MeasureScheduleValueExists extends Validator<MeasureSchedule> {
	...
	class Default implements MeasureScheduleValueExists {
		...
		private ComparisonResult executeDataRule(MeasureSchedule measureSchedule) {
			try {
				return exists(MapperS.of(measureSchedule).<BigDecimal>map("getValue", _measureSchedule -> _measureSchedule.getValue())).or(exists(MapperS.of(measureSchedule).<DatedValue>mapC("getDatedValue", _measureSchedule -> _measureSchedule.getDatedValue())));
			}
			catch (Exception ex) {
