...
public class SecurityLocateMeta implements RosettaMetaData<SecurityLocate> {
	...
	public List<Validator<? super SecurityLocate>> dataRules(ValidatorFactory factory) {
		return Arrays.asList(
			factory.<cdm.event.position.AvailableInventory>create(cdm.event.position.validation.datarule.AvailableInventoryValidPartyRole.class),
			factory.<cdm.event.position.SecurityLocate>create(cdm.event.position.validation.datarule.SecurityLocateRequestOneSecurityMinimum.class)
		);
	}
