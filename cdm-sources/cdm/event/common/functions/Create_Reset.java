	...
	public static class Create_ResetDefault extends Create_Reset {
		...
		protected TradeState.TradeStateBuilder assignOutput(TradeState.TradeStateBuilder reset0, ResetInstruction instruction, TradeState tradeState) {
			...
			if (areEqual(MapperS.of(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<PerformancePayout>map("getPerformancePayout", _payout -> _payout.getPerformancePayout()).resultCount()), MapperS.of(1), CardinalityOperator.All).getOrDefault(false)) {
				final Reset reset1 = resolvePerformanceReset.evaluate(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<PerformancePayout>map("getPerformancePayout", _payout -> _payout.getPerformancePayout()).get(), observation(instruction, tradeState).get(), MapperS.of(instruction).<Date>map("getResetDate", resetInstruction -> resetInstruction.getResetDate()).get());
