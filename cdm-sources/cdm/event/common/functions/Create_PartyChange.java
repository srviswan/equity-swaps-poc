	...
	*/
	...
	public static class Create_PartyChangeDefault extends Create_PartyChange {
		...
		protected TradeState.TradeStateBuilder assignOutput(TradeState.TradeStateBuilder newTrade, Counterparty counterparty, AncillaryParty ancillaryParty, PartyRole partyRole, List<? extends TradeIdentifier> tradeId, TradeState originalTrade) {
			...
			newTrade
				.getOrCreateTrade()
				.addParty(MapperS.of(ancillaryParty).<ReferenceWithMetaParty>mapC("getPartyReference", _ancillaryParty -> _ancillaryParty.getPartyReference()).<Party>map("Type coercion", referenceWithMetaParty -> referenceWithMetaParty.getValue()).getMulti());
			...
			newTrade
				.getOrCreateTrade()
				.addParty(MapperS.of(partyRole).<ReferenceWithMetaParty>map("getPartyReference", _partyRole -> _partyRole.getPartyReference()).<Party>map("Type coercion", referenceWithMetaParty2 -> referenceWithMetaParty2 == null ? null : referenceWithMetaParty2.getValue()).getMulti());
