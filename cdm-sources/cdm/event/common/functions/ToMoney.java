	...
	*/
	...
	public static class ToMoneyDefault extends ToMoney {
		...
		protected Money.MoneyBuilder assignOutput(Money.MoneyBuilder money, Quantity quantity) {
			...
			final FieldWithMetaString fieldWithMetaString = MapperS.of(quantity).<UnitType>map("getUnit", _quantity -> _quantity.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).get();
			money
				.getOrCreateUnit()
