...
public class InterestRateIndexTypeFormatValidator implements Validator<InterestRateIndex> {
	...
	public ValidationResult<InterestRateIndex> validate(RosettaPath path, InterestRateIndex o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InterestRateIndex", ValidationType.TYPE_FORMAT, "InterestRateIndex", path, "", error);
		}
		return success("InterestRateIndex", ValidationType.TYPE_FORMAT, "InterestRateIndex", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestRateIndex", ValidationType.TYPE_FORMAT, "InterestRateIndex", path, "", res.getError());
				}
				return success("InterestRateIndex", ValidationType.TYPE_FORMAT, "InterestRateIndex", path, "");
			})
			.collect(toList());
