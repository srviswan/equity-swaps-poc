...
public class EquityIndexValidator implements Validator<EquityIndex> {
	...
	public ValidationResult<EquityIndex> validate(RosettaPath path, EquityIndex o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EquityIndex", ValidationType.CARDINALITY, "EquityIndex", path, "", error);
		}
		return success("EquityIndex", ValidationType.CARDINALITY, "EquityIndex", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityIndex", ValidationType.CARDINALITY, "EquityIndex", path, "", res.getError());
				}
				return success("EquityIndex", ValidationType.CARDINALITY, "EquityIndex", path, "");
			})
			.collect(toList());
