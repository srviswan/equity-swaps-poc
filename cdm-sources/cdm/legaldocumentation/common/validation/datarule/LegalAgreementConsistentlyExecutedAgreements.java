...
public interface LegalAgreementConsistentlyExecutedAgreements extends Validator<LegalAgreement> {
	...
	class Default implements LegalAgreementConsistentlyExecutedAgreements {
		...
		private ComparisonResult executeDataRule(LegalAgreement legalAgreement) {
			try {
				if (exists(MapperS.of(legalAgreement).<LegalAgreement>mapC("getRelatedAgreements", _legalAgreement -> _legalAgreement.getRelatedAgreements())).and(exists(MapperS.of(legalAgreement).<Date>map("getAgreementDate", _legalAgreement -> _legalAgreement.getAgreementDate()))).getOrDefault(false)) {
