...
class Reset(BaseDataClass):
    ...
    """
    resetValue: cdm.observable.asset.Price.Price = Field(..., description="Specifies the reset or fixing value. The fixing value could be a cash price, interest rate, or other value.")
    """
    Specifies the reset or fixing value. The fixing value could be a cash price, interest rate, or other value.
    """
    resetDate: datetime.date = Field(..., description="Specifies the date on which the reset occurred.")
    """
    ...
    """
    observations: List[AttributeWithReference | cdm.observable.event.Observation.Observation] = Field([], description="Represents an audit of the observations used to produce the reset value. If multiple observations were necessary to produce the reset value, the aggregation method should be defined on the payout.")
    """
    ...
    averagingMethodology: Optional[cdm.product.template.AveragingCalculation.AveragingCalculation] = Field(None, description="Identifies the aggregation method to use in the case where multiple observations are used to compute the reset value and the method is not defined in a payout.")
    """
    ...
    def condition_0_AveragingMethodologyExists(self):
        ...
        item = self
        def _then_fn0():
            return rosetta_attr_exists(rosetta_resolve_attr(self, "averagingMethodology"))
        
        def _else_fn0():
            return True
        
        return if_cond_fn(all_elements(rosetta_count(rosetta_resolve_attr(self, "observations")), ">", 1), _then_fn0, _else_fn0)
