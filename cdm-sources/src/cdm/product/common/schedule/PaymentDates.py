...
class PaymentDates(BaseDataClass):
    ...
    """
    paymentFrequency: Optional[cdm.base.datetime.Frequency.Frequency] = Field(None, description="The frequency at which regular payment dates occur. If the payment frequency is equal to the frequency defined in the calculation period dates component then one calculation period contributes to each payment amount. If the payment frequency is less frequent than the frequency defined in the calculation period dates component then more than one calculation period will contribute to the payment amount. A payment frequency more frequent than the calculation period frequency or one that is not a multiple of the calculation period frequency is invalid. If the payment frequency is of value T (term), the period is defined by the effectiveDate and the terminationDate.")
    """
    ...
    """
    lastRegularPaymentDate: Optional[datetime.date] = Field(None, description="The last regular payment date when specified as a date, as in the FpML interest rate construct. FpML specifies that this date may be subject to adjustment in accordance with any business day convention specified in the paymentDatesAdjustments attribute.")
    """
    ...
    """
    paymentDateSchedule: Optional[cdm.product.common.schedule.PaymentDateSchedule.PaymentDateSchedule] = Field(None, description="The payment dates when specified as relative to a set of dates specified somewhere else in the instance document/transaction, e.g. the valuation dates as typically the case for equity swaps, or when specified as a calculation period schedule.")
    """
    ...
    """
    paymentDatesAdjustments: Optional[cdm.base.datetime.BusinessDayAdjustments.BusinessDayAdjustments] = Field(None, description="The definition of the business day convention and financial business centers used for adjusting the payment date if it would otherwise fall on a day that is not a business day in the specified business center.")
    """
    ...
    def condition_0_FpML_ird_35_cd_31(self):
        ...
        item = self
        def _then_fn0():
            return all_elements(rosetta_resolve_attr(self, "firstPaymentDate"), "<", rosetta_resolve_attr(self, "lastRegularPaymentDate"))
        
        def _else_fn0():
            return True
        
        return if_cond_fn((rosetta_attr_exists(rosetta_resolve_attr(self, "firstPaymentDate")) and rosetta_attr_exists(rosetta_resolve_attr(self, "lastRegularPaymentDate"))), _then_fn0, _else_fn0)
    ...
    def condition_1_NonZeroPeriodMultiplier(self):
        ...
        item = self
        def _then_fn0():
            return any_elements(rosetta_resolve_attr(rosetta_resolve_attr(self, "paymentDaysOffset"), "periodMultiplier"), "<>", 0)
        
        def _else_fn0():
            return True
        
        return if_cond_fn(rosetta_attr_exists(rosetta_resolve_attr(self, "paymentDaysOffset")), _then_fn0, _else_fn0)
