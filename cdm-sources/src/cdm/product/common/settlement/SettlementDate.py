...
class SettlementDate(BaseDataClass):
    ...
    """
    adjustableOrRelativeDate: Optional[cdm.base.datetime.AdjustableOrAdjustedOrRelativeDate.AdjustableOrAdjustedOrRelativeDate] = Field(None, description="A single settlement date subject to adjustment or specified as relative to another date (e.g. the trade date). This attribute was formerly part of 'SettlementTerms', which is now being harmonised to include a common 'SettlementDate', as inherited from 'SettlementBase'.")
    """
    ...
    """
    valueDate: Optional[datetime.date] = Field(None, description="The settlement date for a forward settling product. For Foreign Exchange contracts, this represents a common settlement date between both currency legs. To specify different settlement dates for each currency leg, see the ForeignExchange class. This attribute was formerly part of 'SettlementTerms', which is now being harmonised to include a common 'SettlementDate', as inherited from 'SettlementBase'.")
    """
    ...
    """
    businessDateRange: Optional[cdm.base.datetime.BusinessDateRange.BusinessDateRange] = Field(None, description="A range of contiguous business days. This attribute is meant to be merged with the 'settlementDate' at some future point once we refactor 'Date' to use a single complex type across the model. This attributes was formerly part of 'CashSettlementPaymentDate', as included into 'OptionCashSettlement' (which is now merged into a unique 'CashSettlementTerms' data type.")
    """
    ...
    def condition_0_BusinessDays(self):
        ...
        item = self
        def _then_fn0():
            return all_elements(rosetta_resolve_attr(self, "cashSettlementBusinessDays"), ">=", 0)
        
        def _else_fn0():
            return (((rosetta_attr_exists(rosetta_resolve_attr(self, "adjustableOrRelativeDate")) or rosetta_attr_exists(rosetta_resolve_attr(self, "valueDate"))) or rosetta_attr_exists(rosetta_resolve_attr(self, "adjustableDates"))) or rosetta_attr_exists(rosetta_resolve_attr(self, "businessDateRange")))
        
        return if_cond_fn(rosetta_attr_exists(rosetta_resolve_attr(self, "cashSettlementBusinessDays")), _then_fn0, _else_fn0)
