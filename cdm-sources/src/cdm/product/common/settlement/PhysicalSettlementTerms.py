...
class PhysicalSettlementTerms(BaseDataClass):
    ...
    """
    deliverableObligations: Optional[cdm.product.common.settlement.DeliverableObligations.DeliverableObligations] = Field(None, description="This element contains all the ISDA terms relevant to defining the deliverable obligations.")
    """
    ...
    def condition_0_PredeterminedClearingOrganizationParty(self):
        item = self
        def _then_fn0():
            return all_elements(rosetta_resolve_attr(self, "predeterminedClearingOrganizationParty"), "=", rosetta_resolve_attr(AncillaryRoleEnum, "PREDETERMINED_CLEARING_ORGANIZATION_PARTY"))
        
        def _else_fn0():
            return True
        
        return if_cond_fn(rosetta_attr_exists(rosetta_resolve_attr(self, "predeterminedClearingOrganizationParty")), _then_fn0, _else_fn0)
